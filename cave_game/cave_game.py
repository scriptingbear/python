...
shelve_data = shelve.open('game_data')

loc = 1 
while True:
    availableExits = ", ".join(shelve_data['locations'][loc]["exits"].keys())

    print(shelve_data['locations'][loc]["desc"])
    print('loc = {}'.format(loc))
    if loc == '0':
        break
    else:
        allExits = shelve_data['locations'][loc]["exits"].copy()
        allExits.update(shelve_data['locations'][loc]["namedExits"])
    ...
    if len(direction) > 1:  # more than 1 letter, so check vocab
        ...
        for word in words:
            if word in shelve_data['vocabulary']:   # does it contain a word we know?
                direction = shelve_data['vocabulary'][word]
                break

    if direction in allExits:
        loc = allExits[direction]
    elif direction in ['Q', 'q', '0']:
        #must have a way to exit when loc is an invalid value is entered otherwise the loop
        #runs infinitely
        break
    else:
